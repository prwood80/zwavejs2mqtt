...
function onNodeAdded (zwaveNode) {
  ...
  if (this.driverReady) {
    addNode.call(this, zwaveNode)
    this.sendToSocket(socketEvents.nodeAdded, this.nodes[zwaveNode.id])
  }
...
function onNodeNotification (zwaveNode, notificationLabel, parameters) {
  ...
  if (typeof parameters === 'object') {
    ...
    this.emit('notification', node, notificationLabel, parameters)
  }
...
function addNode (zwaveNode) {
  ...
  if (this.nodes[nodeId] && !this.nodes[nodeId].failed) {
    logger.error(
 ...
 * @returns {import('zwave-js').Association[]} The array of associations
 */
 ...
 * @param {number} nodeId Zwave node id
 * @param {number} groupId Zwave node group Id
 * @param {import('zwave-js').Association[]} associations Array of associations
 */
 ...
 *
 * @returns {number[][]} The nodes array where `nodeId` is the array index and the value is the array
